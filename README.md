...
# PCB
Big Keyboard's PCB has got a couple of parts to it - a Raspberry Pi Pico (no wireless) and, diodes, and keys (pretty simple). The Diodes are SMD, while the Cherry MX keys and Pi Pico are THT.
...
# FIRMWARE/SOFTWARE
Pretty simple firmware, used QMK (had prior experience). Haven't coded the function of the function keys (f1, f2, etc) when not in function mode, haven't decided what I want them to do.
